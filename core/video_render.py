...
def _vt_available() -> bool:
    """Return True if hevc_videotoolbox (Apple Silicon) is usable."""
    try:
        r = _sp.run(
            ["ffmpeg", "-hide_banner", "-f", "lavfi", "-i", "color=c=black:s=16x16:d=0.1",
...
def _vt_codec_args() -> list[str]:
    """
    Return ffmpeg codec args for clip encoding.

    Benchmark result (Apple Silicon M-series, 1920×1080 zoompan + 3-input filter):
      libx264_ultrafast: 2.3s/clip  7.9 MB  — FASTEST (filter is bottleneck, not codec)
      hevc_videotoolbox: 3.0s/clip  6.1 MB  — 24% smaller files, 30% slower
      h264_videotoolbox: 3.1s/clip  6.5 MB  — no advantage over libx264

    Conclusion: zoompan filter is CPU-bound; VideoToolbox doesn't accelerate it.
    libx264_ultrafast wins on speed. Keep hevc path available if file size matters.
    """
...
def renderizar_video_fast(
    ...
    def render_clip(args):
        ...
        cmd = [
            ...
            "-t", str(dur),
            "-r", str(fps),
            # VideoToolbox (Apple Silicon hardware encoder) — 10-15× faster than libx264.
            # Falls back to libx264 ultrafast on non-Apple systems.
            *_vt_codec_args(),
    ...
    if musica_path and os.path.exists(musica_path):
        ...
        #
        # Optimization: if pre-normalized AAC exists (generated by generate_playlist cache),
        # skip loudnorm here — reduces mux time from ~6 min to ~5 sec on 120min renders.
        _prenorm = (
            musica_path.rsplit(".wav", 1)[0] + "_norm.aac"
            if musica_path.endswith(".wav") else None
        )
        if _prenorm and os.path.exists(_prenorm):
            _audio_in  = _prenorm
            _audio_af  = "acopy"   # already normalized + faded — no processing needed
            print(f"  [mux] Pre-normalized audio → fast mux (skipping loudnorm)")
        else:
            _audio_in  = musica_path
            _audio_af  = "afade=t=in:st=0:d=5,loudnorm=I=-16:TP=-1.5:LRA=11"

        subprocess.run(
            ["ffmpeg", "-y",
             "-i", concat_silent, "-i", _audio_in,
             "-c:v", "copy",
             "-af", _audio_af,
             "-c:a", "aac", "-b:a", "192k",
