...
# Orden estratégico — hooks más fuertes primero
UPLOAD_ORDER = [
...
def build_schedule(start_date: datetime) -> list[dict]:
    ...
    for i, short_id in enumerate(UPLOAD_ORDER):
        ...
        mp4 = list((PROJECT_DIR / "output" / "shorts" / "batch_v6").glob(f"short_{short_id}_*.mp4"))
        ...
        schedule.append({
            ...
            "thumb":       str(PROJECT_DIR / "output" / "shorts" / "batch_v6" / "thumbs" / f"{short_id}.jpg"),
...
def main():
    ...
    print(f"\n{'─'*50}")
    print(f"  {mode} — 10 Shorts · 1/día · 5am MTY")
    ...
    for entry in schedule:
        print(f"[{entry['day']:2}/10] {entry['id']}")
    ...
    print(f"{'─'*50}")
    print(f"✅ {ok}/10 programados")
    print(f"   Plan guardado: {SCHEDULE_PATH}")
    if not args.dry_run:
        print(f"\n   YouTube publica automáticamente entre {start.strftime('%Y-%m-%d')} y {(start+timedelta(days=9)).strftime('%Y-%m-%d')} a las 5am MTY")
